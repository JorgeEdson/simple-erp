...
namespace simple_erp.Testes.Modulos.Suprimentos.Repositories
{
    ...
    /// Suprimentos. Cria o schema do modelo EF e trunca a tabela entre testes. Requer
    /// Docker; as classes que a usam levam [Trait("Categoria","Integracao")].
    /// </summary>
    public sealed class PostgresSuprimentosFixture : IAsyncLifetime
    {
        ...
        {
            await using var contexto = CriarContexto();
            await contexto.Database.ExecuteSqlRawAsync("TRUNCATE TABLE suprimentos.pedidos_de_compra");
        }
    }
}
