...
namespace simple_erp.Testes.Modulos.Estoque.Repositories
{
    ...
    {
        private static readonly Guid IdProduto = new Guid("00000000-0000-0000-0000-202607210001");
        ...
        public async Task AdicionarEObterPorProduto_DevePersistirERecuperarOSaldo()
        {
            var saldo = SaldoDeEstoqueBuilder.Novo()
                .ComId(new Guid("00000000-0000-0000-0000-202607210500"))
            ...
            recuperado!.IdProduto.Should().Be(IdProduto);
        ...
        public async Task ObterPorProdutoAsync_DeveRetornarSucessoComNulo_QuandoNaoExiste()
        {
            ...
            var resultado = await repositorio.ObterPorProdutoAsync(IdDe(new Guid("00000000-0000-0000-0000-999999999999")));

            resultado.EhSucesso.Should().BeTrue();
            resultado.Instancia.Should().BeNull();

            (await repositorio.ExistePorProdutoAsync(IdDe(new Guid("00000000-0000-0000-0000-999999999999")))).Instancia.Should().BeFalse();
        }
        ...
        public async Task ObterPorProdutoAsync_DeveRetornarEntidadeRastreada_PermitindoMovimentarEAtualizar()
        {
            await using (var contexto = _fixture.CriarContexto())
            {
                ...
                await repositorio.AdicionarAsync(
                    SaldoDeEstoqueBuilder.Novo().ComId(new Guid("00000000-0000-0000-0000-202607210510")).ComIdProduto(IdProduto)
            ...
            await using (var contexto = _fixture.CriarContexto())
            {
                ...
                var origem = OrigemDaMovimentacao.TentarCriar(TipoOrigemMovimentacao.Venda, new Guid("00000000-0000-0000-0000-000000000555")).Instancia;
        ...
        public async Task Adicionar_DoisSaldosParaOMesmoProduto_DeveViolarOIndiceUnico()
        {
            ...
            await repositorio.AdicionarAsync(
                SaldoDeEstoqueBuilder.Novo().ComId(new Guid("00000000-0000-0000-0000-202607210520")).ComIdProduto(IdProduto).Criar());
            await repositorio.AdicionarAsync(
                SaldoDeEstoqueBuilder.Novo().ComId(new Guid("00000000-0000-0000-0000-202607210521")).ComIdProduto(IdProduto).Criar());
